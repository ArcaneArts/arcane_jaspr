...
/// Translates an [ArcaneDecoration] into Neon CSS.
///
/// Neon renders elevation as a colored glow at higher levels (its signature
/// look) and falls back to ambient `--shadow-*` at low levels. When present,
/// `shadowColor` tints the glow for this element only.
Map<String, String> neonDecorationStyles(ArcaneDecoration? decoration) {
  final Elevation? elevation = decoration?.elevation;
  if (elevation == null) return const <String, String>{};
  return <String, String>{
    'box-shadow': _neonElevation(elevation, decoration?.shadowColor),
  };
}

String _neonElevation(Elevation elevation, String? shadowColor) {
  final String glow = shadowColor != null
      ? '0 0 18px $shadowColor, 0 0 36px ${_fade(shadowColor)}'
      : 'var(--neon-glow, var(--shadow-lg))';
  return switch (elevation) {
    Elevation.none => 'none',
    Elevation.xs => 'var(--shadow-xs)',
    Elevation.sm => 'var(--shadow-sm)',
    Elevation.md => 'var(--neon-glow-sm, var(--shadow-md))',
    Elevation.lg => glow,
    Elevation.xl => glow,
  };
}
